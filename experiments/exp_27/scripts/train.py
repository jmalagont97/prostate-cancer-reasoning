"""exp_27 — MemKDM with training restricted to sigma-only, frozen unimodally. See ../DESIGN.md and
../IMPLEMENTATION.md.

Every MemKDM in exp_23-26 trained some subset of {x, y, w, sigma} via 300 epochs of Adam on each fold's
own training labels, in BOTH Phase A (MCCV hyperparameter search) and Phase B (LOOCV final evaluation) —
so Phase B was never a pure frozen-model evaluation, it was 88 independent small fits. This experiment
restricts training to the one place it is well-defined for a memory-based model: `sigma` (a scalar per
modality, not tied to which patients are in memory), fit once unimodally in Stage 1 and frozen everywhere
else (Stage 2 and Phase B are then pure evaluation, zero gradient steps). `x`, `y`, `w` are never trained
anywhere (`x_train=y_train=w_train=False` throughout) — per DESIGN.md Sec 2.1/2.2, w_train=True was never
grid-searched to begin with, and x_train/y_train consistently selected False when exp_23/25/26 DID search
them. `sigma_scale` (exp_25/26's Stage-2 grid dimension) is dropped as redundant with Stage 1's sigma_mult
grid (DESIGN.md Sec 2.5). Required one `src/methods/mem_kdm.py` fix (DESIGN.md Sec 2.4): `MemKDM.fit()`
now skips optimizer construction/the training loop when the model has zero trainable parameters (Stage 2
and Phase B's case here) instead of crashing on `torch.optim.Adam([])`.

Everything not called out in DESIGN.md/IMPLEMENTATION.md is copied verbatim from `exp_26/scripts/train.py`,
including its own inherited build-time simplifications (no per-config feature caching; fusion-weight stash
via a small post-hoc re-fit pass) — see that file's docstring.
"""
...
MRI_GRID = [{"rep": rep, "sigma_mult": sm} for rep in ["raw_l2", "pca90_l2"] for sm in [0.5, 1.0, 2.0]]
TXT_GRID = [{"rep": (mf, 0.90), "sigma_mult": sm} for mf in [500, 2000, None] for sm in [0.5, 1.0, 2.0]]
assert len(TAB_GRID) == 4 and len(MRI_GRID) == 6 and len(TXT_GRID) == 9

STAGE2_GRID = [{}]  # nothing left to search: sigma is frozen from Stage 1, x/y/w always False (DESIGN.md Sec 2.3/2.5)
...
def main(smoke: bool = False):
    ...
    print(f"[exp_27] cohort N={n}, yes={int(cohort.y_binary.sum())}, no={int((1 - cohort.y_binary).sum())}")

    print("[exp_27] spaCy cleaning text corpus (once, cohort-level)...")
    ...
    # invariant on an 88-patient cohort (exp_25/exp_24 hardcoded n_conf_splits=100 for the identical reason).
    ...
    print(f"[exp_27] {'SMOKE MODE: ' if smoke else ''}{len(SPLITS_3T)} MCCV splits, {len(LOOCV_FOLDS)} LOOCV folds")
    ...
    # Unchanged from exp_26 (DESIGN.md Sec 4) — a divergent-by-design historical oracle, not part of this
    # experiment's own model (default trainable=True, w_train=True, matching exp_23's original config).
    print("[exp_27] Step 0: reproduction gate...")
    ...
    print(f"[exp_27] Step 0 PASSED: macro_f1={repro_metrics['macro_f1']!r}, "
    ...
    print("[exp_27] Step 0: check_roundtrip passed for tab / mri(pca90_l2) / txt(2000,0.9)")

    # ---------------------------------------------------------------- 1.4 Stage 1 Phase A
    print("[exp_27] Stage 1 Phase A...")
    ...
    for name, grid in [("tab", TAB_GRID), ("mri", MRI_GRID), ("txt", TXT_GRID)]:
        ...
        print(f"[exp_27] Stage 1 {name}: best mean_macro_f1={STAGE1_BEST[name]['mean_macro_f1']:.4f}, cfg={STAGE1_BEST[name]}")
    ...
    print("[exp_27] Fusion-weight search (post-hoc re-fit of the 3 Stage-1 winners across 100 splits)...")
    ...
    def winner_val_probs(name):
        ...
        for split_idx, train_idx, val_idx in SPLITS_3T:
            X_tr, X_va = bm(name, rep, train_idx, val_idx)
            m = MemKDM(kernels={name: KernelSpec(sigma_mult=best["sigma_mult"])}, encoders={name: EncoderSpec("identity")},
                       x_train=False, y_train=False, w_train=False, epochs=300, lr=1e-3, seed=0)
            m.fit({name: X_tr}, make_targets(train_idx))
    ...
    print(f"[exp_27] fusion weights: {fusion_weights}")

    # ---------------------------------------------------------------- 1.5 Stage 2 Phase A
    print("[exp_27] Stage 2 Phase A...")

    def build_joint_kernels_encoders(subset):
        """Sigma is frozen from Stage 1's mean fitted value (DESIGN.md Sec 2.3) — no per-fold
        `_sigma_from_knn` recomputation, no gradient refinement (trainable=False). Identical for
        Stage 2's evaluation pass and every Phase B LOOCV fold."""
    ...
    for subset in CONDITIONS:
        ...
        print(f"[exp_27] Stage 2 {key}: mean_macro_f1={STAGE2_BEST[key]['mean_macro_f1']:.4f}")
    ...
    print(f"[exp_27] Stage 2 confidence_arm: mean_macro_f1={STAGE2_BEST['confidence_arm']['mean_macro_f1']:.4f}")
    ...
    print("[exp_27] Phase B (LOOCV)...")

    def phase_b_condition(subset, n_seeds, label_smoothing=0.0):
    ...
    for mod in ["tab", "mri", "txt"]:
        unimodal_oof[mod] = phase_b_condition([mod], n_seeds=1)  # no training at all -> deterministic
        print(f"[exp_27] Phase B unimodal_{mod}: macro_f1={np.mean(unimodal_oof[mod]['per_seed_macro_f1']):.4f}")

    joint_oof = {}
    for key, subset in JOINT_KEYS.items():
        joint_oof[key] = phase_b_condition(subset, n_seeds=1)
        print(f"[exp_27] Phase B {key}: macro_f1={np.mean(joint_oof[key]['per_seed_macro_f1']):.4f}")

    confidence_arm_oof = phase_b_condition(["tab", "mri", "txt"], n_seeds=1, label_smoothing=0.10)
    print("[exp_27] Phase B confidence_arm: done")
    ...
    # ---------------------------------------------------------------- figures
    print("[exp_27] Figures...")
    try:
        for name, df_grid in [("tab", STAGE1_GRIDS[0]), ("mri", STAGE1_GRIDS[1]), ("txt", STAGE1_GRIDS[2])]:
            # "modality" (constant per df_grid slice) stands in for tab's missing grouping dimension —
            # pandas' groupby rejects an empty group_cols list ("No group keys passed!"), and tab has no
            # `rep` column to group by since y_train/encoder are gone (Sec 2.2/2.5).
            group_cols = ["rep"] if "rep" in df_grid.columns else ["modality"]
            plot_grid_search_curves(df_grid, x_col="sigma_mult", y_col="mean_macro_f1", group_cols=group_cols,
                                     title=f"exp_27 Stage 1 {name} grid search", out_path=FIG_DIR / f"stage1_grid_search_curves_{name}.png")
        plot_confusion_matrix(y_true_eval, y_pred_trimodal, labels=["No", "Yes"],
                               title="exp_27 joint_trimodal LOOCV", out_path=FIG_DIR / "confusion_matrix.png")
        plot_roc_curves({"joint_trimodal": ALL_CONDITIONS["joint_trimodal"][idx, 1],
                         ...
                         y_true_eval, title="exp_27 ROC overlay", out_path=FIG_DIR / "roc_curve.png")
        plot_signal_scatter(CONDITION_SIGNALS["joint_trimodal"]["h_aleatoric"][idx], CONDITION_SIGNALS["joint_trimodal"]["h_epistemic"][idx],
                             ...
                             xlabel="h_aleatoric", ylabel="h_epistemic", title="exp_27 joint_trimodal particle signals",
        ...
        print(f"[exp_27] best confidence row (for reference, not re-plotted): {best_conf_row['condition']}/{best_conf_row['head']} "
        ...
        print(f"[exp_27] WARNING: figure generation failed: {e}")

    record_git_commit(RESULTS_DIR)
    elapsed = time.time() - t_start
    print(f"[exp_27] DONE in {elapsed / 60:.1f} min. Results in {RESULTS_DIR}")
