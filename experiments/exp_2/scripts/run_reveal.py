"""exp_2: reveal-sequence model, 2 conditions = {count, flags}.

Reuses chimera_task1.train_reasoning's make_classifier and the CV/scoring
pattern from its eval_reveal() (adapted here to return values instead of
only printing) -- no changes to train_reasoning.py. Feature frame comes from
...
Run with the project's own .venv (from the project root):
    .venv/Scripts/python.exe experiments/exp_2/scripts/run_reveal.py
"""
...
def write_metrics(condition: str, metrics: dict) -> None:
    ...
    payload = {
        ...
        "features": "11 official Task-1 variables",
        **metrics,
    }
...
def main() -> None:
    ...
    for treatment in ("count", "flags"):
        ...
        write_metrics(f"reveal_{treatment}", {"comorbidity_treatment": treatment, **run_condition(X, ann)})
