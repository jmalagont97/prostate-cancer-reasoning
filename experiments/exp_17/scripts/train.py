...
def compute_ici(p1, p2, p3):
    ...
    # Composite Reliability Index (ICI) - Identical to exp_9, exp_10, exp_11
...
def main():
    ...
    exp_dir = project_root / "experiments" / "exp_17"
    ...
    print("Loading out-of-fold soft predictions from exp_16 and clinical reasoning targets...")
    df_oof = pd.read_csv(project_root / "experiments" / "exp_16" / "results" / "oof_predictions.csv")
    ...
    p_tab = df_oof_labeled["prob_tabular_fuzzy"].values
    p_mri = df_oof_labeled["prob_mri_fuzzy"].values
    p_text = df_oof_labeled["prob_text_fuzzy"].values

    # Compute Composite Fuzzy ICI (Identical composite formula to exp_9, exp_10, exp_11)
    ici_fuzzy, p_mean, std_p, delta_margin = compute_ici(p_tab, p_mri, p_text)
    ...
    for split_idx in range(n_splits):
        ...
        X_tr = ici_fuzzy[train_mask].reshape(-1, 1)
    ...
    print(f"Learned Composite Balanced Meta-Thresholds: t1 = {meta_t1:.4f} (+/- {std_t1:.4f}), t2 = {meta_t2:.4f} (+/- {std_t2:.4f})")

    # Plot MCCV Thresholds Histogram
    plt.figure(figsize=(9, 5))
    plt.hist(ici_fuzzy[y_conf == 0], bins=15, alpha=0.6, label="Uncertain", color="red")
    plt.hist(ici_fuzzy[y_conf == 1], bins=15, alpha=0.6, label="Borderline", color="orange")
    plt.hist(ici_fuzzy[y_conf == 2], bins=15, alpha=0.6, label="Clear", color="green")
    ...
    plt.title("Composite Fuzzy ICI Distribution & Learned Balanced Meta-Thresholds", fontsize=11, fontweight="bold")
    plt.xlabel("Composite Reliability Index (ICI)", fontsize=10)
    ...
    plt.title(f"Class-Weighted Composite Fuzzy ICI 3-Class Confusion Matrix (Macro-F1: {macro_f1:.4f})", fontsize=11, fontweight="bold")
    plt.tight_layout()
    ...
    with open(summary_md_path, "w") as f:
        f.write("# Class-Weighted Composite Fuzzy ICI Diagnostic Confidence Prediction (exp_17) Summary Report\n\n")
        f.write("**Date**: 2026-08-05  \n")
        f.write(f"**Model**: Class-Weighted Decision Tree Meta-Thresholding on Composite Fuzzy ICI (`class_weight='balanced'`)  \n")
        ...
        f.write("## Comparison against Baseline (exp_10 Hard Composite ICI Meta-Thresholding)\n")
        f.write("| Experiment | ICI Formulation | Decision Tree Weighting | Meta \bar{\\tau}_1 | Meta \bar{\\tau}_2 | LOOCV Macro-F1 | LOOCV Accuracy | Spearman \rho |\n")
        f.write("|:---|:---:|:---:|:---:|:---:|:---:|:---:|:---:|\n")
        f.write(f"| **`exp_10` (Hard Composite ICI)** | Discrete Hard | Balanced | 0.0669 | 0.2960 | **0.3691** | **39.77%** | **0.1228** |\n")
        f.write(f"| **`exp_17` (Fuzzy Composite ICI)** | **Continuous Soft** | **Balanced** | **{meta_t1:.4f}** | **{meta_t2:.4f}** | **{macro_f1:.4f}** | **{acc*100:.2f}%** | **{rho_val:.4f}** |\n")
