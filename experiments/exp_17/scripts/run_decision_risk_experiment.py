#!/usr/bin/env python3
"""
exp_17: Decision Risk Theory for Clinical Confidence Prediction

Predicts target_confidence (uncertain/borderline/clear) using a 1D DecisionTreeClassifier
on a continuous Decision Risk metric Ω(c_fn, λ) derived from the binary biopsy probabilities
of the frozen multimodal ensemble (ConfidenceWeightedKNN for T, M, X).
...
RESULTS_DIR = ROOT / "experiments" / "exp_17" / "results"
FIGURES_DIR = ROOT / "experiments" / "exp_17" / "reports" / "figures"
...
def compute_decision_risk(p_T, p_M, p_X, c_fn, lam):
    """Computes continuous decision risk Ω(c_fn, λ).

    c_fp = 1 - c_fn
    R_margen = min(p_bar * c_fn, (1 - p_bar) * c_fp) / (c_fn * c_fp)
    R_conflicto = 2 * std(p_T, p_M, p_X)
    Ω = (1 - λ) * R_margen + λ * R_conflicto
    """
    c_fp = 1.0 - c_fn
...
def compute_moe_abs(y_true, y_pred):
    """Macro Mean Ordinal Error: (1/3) * Σ_c mean_{i:y_i=c}(|pred_i - true_i|/2)."""
...
def verify_tree_structure(tree):
    """Returns True if tree has max_depth=2 structure (2 internal nodes, 3 leaves)."""
    n_nodes = tree.tree_.node_count
    n_leaves = np.sum(tree.tree_.children_left == -1)
    return (n_nodes == 5) and (n_leaves == 3)
...
def process_outer_fold(train_idx, eval_idx, X_tab_raw, X_mri_raw, preprocessed_texts, y_binary, conf_weights):
    # 1. Inner OOF on train set for leak-safe Ω_train
    ...
    y_train_conf = conf_weights[train_idx]
    ...
    # 2. Fit base models on FULL train_idx, predict on eval_idx
    X_tr_tab, d_cols, ohe, scaler, c_kept, n_cols = build_features_train(
...
def main():
    ...
    print("=" * 80)
    print("exp_17: Decision Risk Theory for Clinical Confidence Prediction")
    print("=" * 80)
    ...
    # Class breakdown
    class_counts = {c: int(np.sum(y_conf_ord == ORD_MAP[c])) for c in ORD_NAMES}
    ...
    for split_idx in range(TOTAL_MCCV_FOLDS):
        ...
        mccv_base_probs.append({
            ...
            "train_idx": train_idx,
            "val_idx": val_idx,
            "inner_p": inner_p,  # (p_T, p_M, p_X) for train_idx
            "eval_p": eval_p,    # (p_T, p_M, p_X) for val_idx
        })
    ...
    # ══════════════════════════════════════════════════════════════════════════
    # Phase 3: Grid Search over 50 Decision Risk Conditions in MCCV
    # ══════════════════════════════════════════════════════════════════════════

    print("\n" + "─" * 80)
    print("Phase 3: Running 50-Condition Grid Search over MCCV...")
    print("─" * 80)
    ...
    config_idx = 0
    total_configs = len(C_FN_VALUES) * len(LAMBDA_VALUES) * len(CLASS_WEIGHT_VALUES)

    for c_fn in C_FN_VALUES:
        for lam in LAMBDA_VALUES:
            for cw in CLASS_WEIGHT_VALUES:
                config_idx += 1
                config_name = f"c_fn_{c_fn:.2f}_lambda_{lam:.2f}_cw_{cw}"
                ...
                for bp in mccv_base_probs:
                    ...
                    tree.fit(omega_train.reshape(-1, 1), y_conf_ord[train_idx])

                    if verify_tree_structure(tree):
                        valid_struct_count += 1

                    pred_val = tree.predict(omega_val.reshape(-1, 1))
                ...
                if config_idx % 10 == 0 or config_idx == total_configs:
                    print(f"  Config {config_idx:02d}/{total_configs} [{config_name}]: pooled_moe={pooled_moe:.4f}, f1={pooled_f1:.4f}, valid_rate={valid_rate:.2f}")
    ...
    # Sort by pooled_moe_abs ASC, then pooled_f1_macro DESC
    eligible_df = eligible_df.sort_values(
        by=["pooled_moe_abs", "pooled_f1_macro"], ascending=[True, False]
    ...
    selected_cw_str = selected_row["class_weight"]
    selected_cw = None if selected_cw_str == "None" else "balanced"

    print("\n" + "★" * 80)
    print(f"SELECTED MCCV CONFIG: {selected_config_name}")
    print(f"  c_fn: {selected_c_fn}, c_fp: {1.0-selected_c_fn:.2f}, lambda: {selected_lam}, class_weight: {selected_cw}")
    print(f"  Pooled MOE_abs: {selected_row['pooled_moe_abs']:.4f} (vs Baseline {baseline_moe_abs:.4f})")
    print(f"  Pooled F1_macro: {selected_row['pooled_f1_macro']:.4f}")
    ...
    for bp in loo_base_probs:
        ...
        tree = DecisionTreeClassifier(
            ...
            min_samples_leaf=5,
            random_state=42,
            class_weight=selected_cw,
        )
        tree.fit(omega_train.reshape(-1, 1), y_conf_ord[train_idx])

        pred_val = tree.predict(omega_val.reshape(-1, 1))[0]
    ...
    # ══════════════════════════════════════════════════════════════════════════
    # Phase 5: Confusion Matrices & Figures
    # ══════════════════════════════════════════════════════════════════════════
    ...
    summary_data = {
        "experiment": "exp_17",
        "description": "Decision Risk Theory for Clinical Confidence Prediction",
