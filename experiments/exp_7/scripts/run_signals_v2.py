"""exp_7: exp_6's run_signals.py, unchanged in every way except the backbone.

Two changes from experiments/exp_6/scripts/run_signals.py:
  (a) X_pre gets log1p-transformed (cli_psa/cli_psad/cli_vol) right after build_preprocessor's
      fit_transform, before the per-fold StandardScaler.
  (b) every fit_kdm_backbone(...) call uses the winning hyperparameters from
      results/hyperparameter_search/winner.json instead of exp_6's fixed defaults.

All 9 conditions, all recalibration logic (isotonic increasing="auto", the make_classifier()
blend, per-factor try/except, the per-factor weights breakdown) are otherwise IDENTICAL to exp_6
-- this script's whole purpose is isolating the backbone change, not introducing new readout
logic. Results written with a _v2 suffix.
...
Run with the project's own .venv (from the project root):
    .venv/Scripts/python.exe experiments/exp_7/scripts/run_signals_v2.py
"""
...
from kdm_backbone_v2 import (  # noqa: E402
    LOG1P_COLUMNS,
    apply_log1p_transform,
    compute_signals,
    fit_kdm_backbone,
    kernel_distance_contribution,
    occlusion_delta,
)
...
def main() -> None:
    ...
    assert pd.notna(y_decision_labels).all(), "exp_7 assumes the annotated 91-case set has a decision label for every row"
    ...
    for repeat in range(N_REPEATS):
        ...
        repeat_metrics["decision_kdm_v2"]["macro_f1"].append(f1_score(y_decision, decision_preds, average="macro"))
    ...
    d = RESULTS_DIR / "decision_kdm_v2"
    d.mkdir(exist_ok=True)
    macro_f1s = repeat_metrics["decision_kdm_v2"]["macro_f1"]
    payload = {
        "condition": "decision_kdm_v2",
        "target": "decision",
        "features": "exp_3 19-column frame (PSA-reduced + MRI-PCA), log1p on psa/psad/vol",
        "model": f"KDM (memory-based, sigma-only trained), tuned config: {winning_config}",
        "macro_f1_mean": round(float(np.mean(macro_f1s)), 3),
        "macro_f1_std": round(float(np.std(macro_f1s)), 3),
        "n_folds": N_REPEATS * N_SPLITS,
        "note": "compare against exp_6's decision_kdm_backbone (macro_f1=0.593)",
    }
    with open(d / "metrics.json", "w") as f:
        json.dump(payload, f, indent=2)
    print(f"[decision_kdm_v2] macro_f1={payload['macro_f1_mean']} (exp_6's decision_kdm_backbone was 0.593)")

    for cond in CONFIDENCE_CONDITIONS:
        d = RESULTS_DIR / f"{cond}_v2"
        d.mkdir(exist_ok=True)
        dists = repeat_metrics[cond]["ordinal_distance"]
        payload = {
            "condition": f"{cond}_v2",
            "target": "confidence",
            "features": "exp_3 19-column frame (PSA-reduced + MRI-PCA), log1p on psa/psad/vol",
            "model": f"exp_7 tuned KDM backbone + signal recalibration, config: {winning_config}",
        ...
        with open(d / "metrics.json", "w") as f:
            json.dump(payload, f, indent=2)
        print(f"[{cond}_v2] ordinal_distance={payload['ordinal_distance_mean']} "
              f"(exp_6's {cond} was compared to baseline 0.527, incumbent 0.468)")
    ...
    for cond in WEIGHT_CONDITIONS:
        ...
        with open(d / "metrics.json", "w") as f:
            json.dump(payload, f, indent=2)
        print(f"[{cond}_v2] mean_ordinal_error={payload['mean_ordinal_error']} "
              f"(exp_6's {cond} was compared to baseline 0.413, incumbent 0.382/0.392)")
