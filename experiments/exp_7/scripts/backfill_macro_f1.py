"""Backfill macro-F1 into exp_7's already-completed confidence/weights results, per this
project's cross-experiment macro-F1 reporting initiative (2026-08-12). Decision already reports
macro_f1 (unchanged). Reveal has no exp_7 condition (added in exp_8).

Reproduces the EXACT same CV loop as run_signals_v2.py (same RANDOM_STATE, log1p transform,
winning hyperparameters from results/hyperparameter_search/winner.json), so existing
ordinal_distance/decisive_set_f1 numbers are reproduced identically -- this script only computes
the ADDITIONAL macro-F1 metric and merges it into the existing metrics.json files.

Run with the project's own .venv (from the project root):
    .venv/Scripts/python.exe experiments/exp_7/scripts/backfill_macro_f1.py
"""
...
from kdm_backbone_v2 import LOG1P_COLUMNS, apply_log1p_transform, compute_signals, fit_kdm_backbone, kernel_distance_contribution, occlusion_delta  # noqa: E402
