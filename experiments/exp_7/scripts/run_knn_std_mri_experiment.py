#!/usr/bin/env python3
"""
exp_7: KNN classifier on Standardized MRI embedding (1024 dimensions).

StandardScaler fitted per fold on training data only, then applied to val/test.
Same grid as exp_6 (72 configs), same metrics, same figure generation.

Usage (tmux session 0):
    python3 experiments/exp_7/scripts/run_knn_std_mri_experiment.py
"""
...
RESULTS = Path("experiments/exp_7/results")
REPORTS = Path("experiments/exp_7/reports")
...
# ══════════════════════════════════════════════════════════════════════════════
# MCCV  (StandardScaler fitted per split on training data)
# ══════════════════════════════════════════════════════════════════════════════

def run_mccv(X_emb, y, confidence, splits_df):
    configs = {}
    for k in K_RANGE:
    ...
    print(f"  {len(configs)} configurations x 50 splits = {n_evals} evaluations")
    all_oof = []
    ...
    for split_idx in range(50):
        ...
        X_train_raw = X_emb[train_idx]
        X_val_raw = X_emb[val_idx]
        y_train = y[train_idx]
        y_val = y[val_idx]
...
# ══════════════════════════════════════════════════════════════════════════════
# LOO  (StandardScaler fitted per fold on 87 training cases)
# ══════════════════════════════════════════════════════════════════════════════
...
def plot_confusion_matrices(cm_mccv, cm_loo, out_dir, best_cn):
    ...
    fig.suptitle(f"exp_7 — Standardized MRI Embedding KNN: {best_cn}",
...
def main():
    print("=" * 70)
    print("exp_7: KNN on Standardized MRI embedding (1024 dims, StandardScaler)")
    print("=" * 70)
    t_start = time.time()
    ...
    X_emb = images_df[emb_cols].values.astype(np.float64)
    ...
    print(f"  MRI embedding shape: {X_emb.shape}")
    print(f"  Preprocessing: StandardScaler per fold (z-score, fit on train only)")
    ...
    print(f"  Raw range: [{X_emb.min():.4f}, {X_emb.max():.4f}]")
    print(f"  Raw mean: {X_emb.mean():.4f}, Raw std: {X_emb.std():.4f}")

    # ── MCCV ───────────────────────────────────────────────────────────────
    print("\n[3/7] MCCV search (72 configs x 50 splits, StandardScaler per split)...")
    configs, summary, oof_mccv = run_mccv(X_emb, y, conf, splits_df)

    # ── Select best ────────────────────────────────────────────────────────
    print("\n[4/7] Selecting best configuration...")
    ...
    # ── LOO ────────────────────────────────────────────────────────────────
    print("\n[5/7] LOO evaluation (best config only, StandardScaler per fold)...")
    oof_loo, loo_metrics = run_loo(X_emb, y, conf, splits_df, best_cfg, best_cn)
    ...
    # ── Confusion matrices ─────────────────────────────────────────────────
    print("\n[6/7] Generating confusion matrix figures...")
    ...
    (out_dir / "hyperparameters.json").write_text(json.dumps({
        ...
        "variant": best_cfg["variant"],
        "preprocessing": "StandardScaler per fold (fit on train only)",
    }, indent=2))
    ...
    sel = {
        ...
        "preprocessing": "StandardScaler per fold (z-score, fit on train only)",
        ...
        "baseline_comparison": {
            "exp6_raw_mccv_f1_macro": 0.5497,
            "exp6_raw_loo_f1_macro": 0.5731,
        },
    }
