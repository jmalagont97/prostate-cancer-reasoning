"""exp_13: ARD-KDM trained DIRECTLY on each factor's weight label -- identical to
run_weights_direct_scalar.py except the backbone (exp_9's fit_kdm_backbone_ard/compute_signals_ard
instead of exp_6's scalar versions). See DESIGN.md Section 2.

Run with the project's own .venv (from the project root):
    .venv/Scripts/python.exe experiments/exp_13/scripts/run_weights_direct_ard.py
"""
...
def run_scope(scope: str, ann: pd.DataFrame, X_full: pd.DataFrame) -> None:
    ...
    payload = {
        ...
        "model": f"ARD-KDM trained directly on each factor's 4-class weight label, fixed config: {ARD_CONFIG}",
