"""exp_13: mandatory held-out check for the directly-trained scalar-KDM weights models, both
scopes, all 9 in-scope factors.

Same fixed 19-case split used since exp_3 (stratified on y_decision, not any weight label -- "the
same split" means the same row indices). MRI-PCA is fit on the train portion only
(mri_pca_train_only, exp_3's own train/test-safe helper, reused unchanged).

Run with the project's own .venv (from the project root):
    .venv/Scripts/python.exe experiments/exp_13/scripts/holdout_eval_weights_direct_scalar.py
"""
