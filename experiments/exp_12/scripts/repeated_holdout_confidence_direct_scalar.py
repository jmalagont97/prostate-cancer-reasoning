"""exp_12 follow-up: 10 repeated 80/20 held-out splits for the scalar direct-trained confidence
KDM, both frames -- the single seed=0 held-out check gave an identical 0.368 ordinal distance for
BOTH frames, the same suspicious pattern exp_11 already showed (there: 0.316 for both frames,
confirmed a lucky outlier). Same discipline, checking again rather than assuming it repeats.

Run with the project's own .venv (from the project root):
    .venv/Scripts/python.exe experiments/exp_12/scripts/repeated_holdout_confidence_direct_scalar.py
"""
...
def main() -> None:
    ...
    out_dir = RESULTS_DIR / "repeated_holdout_confidence_direct_scalar"
    ...
    for frame_tag, r in results.items():
        print(f"{frame_tag}: seed=0 ord_dist={r['seed0_ordinal_distance']} (reported earlier: 0.368) "
