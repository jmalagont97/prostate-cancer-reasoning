"""exp_9: ARD backbone on exp_8's expanded 23-column frame -- decision + confidence + weights.

Identical to run_signals_19col.py except the feature frame (select_exp8_feature_frame instead of
select_exp3_feature_frame) -- see that script's docstring for the shared design rationale. This
is the frame ARD is specifically meant to rescue (DESIGN.md Section 1): exp_8 measured a +0.309
ordinal-distance confidence regression on this exact frame under a scalar-sigma backbone.

Run with the project's own .venv (from the project root):
    .venv/Scripts/python.exe experiments/exp_9/scripts/run_signals_23col.py
"""
...
def main() -> None:
    ...
    payload = {
        ...
        "target": "decision",
        "features": f"exp_8 23-column frame ({FRAME_TAG})",
        ...
        "note": "compare against exp_8's decision_kdm_v3 (0.585 CV, scalar sigma, same frame) and exp_6's 0.593 (19-col reference)",
    }
    with open(d / "metrics.json", "w") as f:
        json.dump(payload, f, indent=2)
    print(f"[decision_kdm_ard_{FRAME_TAG}] macro_f1={payload['macro_f1_mean']} (exp_8 scalar-sigma combined was 0.585)")
