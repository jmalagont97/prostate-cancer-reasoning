...
colors_f1 = plt.cm.magma(np.linspace(0.25, 0.85, len(features)))
bars1 = plt.bar(features, macro_f1s, color=colors_f1, edgecolor="black", linewidth=1.2, width=0.55)

for bar in bars1:
    yval = bar.get_height()
    plt.text(bar.get_x() + bar.get_width()/2.0, yval + 0.003, f"{yval:.4f}", ha='center', va='bottom', fontweight='bold', fontsize=9.5)

plt.title("4-Class Macro-F1 Performance per Clinical Feature (exp_20 Perturbation Ablation)", fontsize=12, fontweight="bold", pad=15)
...
plt.ylim(0, max(macro_f1s) + 0.03)
plt.grid(True, linestyle="--", alpha=0.4, axis="y")
plt.tight_layout()
fig1_path = figures_dir / "histogram_macro_f1_exp20.png"
plt.savefig(fig1_path, dpi=300)
plt.savefig(brain_figures_dir / "histogram_macro_f1_exp20.png", dpi=300)
plt.close()

# 2. Absolute Spearman Rho (|Rho|) Bar Chart / Histogram
plt.figure(figsize=(10, 5.5))
colors_rho = plt.cm.plasma(np.linspace(0.2, 0.85, len(features)))
...
for i, bar in enumerate(bars2):
    ...
    plt.text(bar.get_x() + bar.get_width()/2.0, yval + 0.008, f"|{yval:.4f}| ({sign_str})", ha='center', va='bottom', fontweight='bold', fontsize=9.5)

plt.title("Absolute Spearman Rank Correlation (|Rho|) per Clinical Feature (exp_20 Perturbation Ablation)", fontsize=12, fontweight="bold", pad=15)
plt.xlabel("Clinical Tabular Features", fontsize=11, fontweight="bold")
plt.ylabel("Absolute Spearman Correlation |Rho|", fontsize=11, fontweight="bold")
plt.ylim(0, max(abs_rhos) + 0.06)
plt.grid(True, linestyle="--", alpha=0.4, axis="y")
plt.tight_layout()
fig2_path = figures_dir / "histogram_abs_spearman_rho_exp20.png"
plt.savefig(fig2_path, dpi=300)
plt.savefig(brain_figures_dir / "histogram_abs_spearman_rho_exp20.png", dpi=300)
plt.close()
...
for bar in bars_a:
    yval = bar.get_height()
    axes[0].text(bar.get_x() + bar.get_width()/2.0, yval + 0.003, f"{yval:.4f}", ha='center', va='bottom', fontweight='bold', fontsize=9)
axes[0].set_title("A) 4-Class Macro-F1 Score per Feature (exp_20)", fontsize=12, fontweight="bold")
...
axes[0].set_ylim(0, max(macro_f1s) + 0.03)
...
for i, bar in enumerate(bars_b):
    ...
    raw_val = raw_rhos[i]
    sign_str = "+" if raw_val >= 0 else "-"
    axes[1].text(bar.get_x() + bar.get_width()/2.0, yval + 0.008, f"|{yval:.4f}| ({sign_str})", ha='center', va='bottom', fontweight='bold', fontsize=9)
axes[1].set_title("B) Absolute Spearman Correlation |Rho| per Feature (exp_20)", fontsize=12, fontweight="bold")
...
plt.suptitle("Clinical Feature Relevance Attribution Summary via Mode/Median Perturbation (exp_20)", fontsize=14, fontweight="bold", y=1.02)
plt.tight_layout()
fig_combined_path = figures_dir / "histograms_combined_f1_rho_exp20.png"
plt.savefig(fig_combined_path, dpi=300)
plt.savefig(brain_figures_dir / "histograms_combined_f1_rho_exp20.png", dpi=300)
plt.close()
