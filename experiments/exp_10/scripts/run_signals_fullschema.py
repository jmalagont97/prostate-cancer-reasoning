"""exp_10: ARD backbone on the full-schema (Frame.md, 48-column) frame -- decision + confidence +
weights. Structural copy of experiments/exp_9/scripts/run_signals_23col.py, with
features_fullschema's frame/preprocessing/factor-grouping functions instead of
select_exp8_feature_frame/build_preprocessor+StandardScaler/chimera_task1.features's
restricted_feature_group (see DESIGN.md Section 1 and this experiment's IMPLEMENTATION.md,
findings #1-#2).

Run with the project's own .venv (from the project root):
    .venv/Scripts/python.exe experiments/exp_10/scripts/run_signals_fullschema.py
"""
...
from features_fullschema import (  # noqa: E402
    fit_transform_fullschema,
    restricted_feature_group_fullschema,
    select_exp10_feature_frame,
)
...
def main() -> None:
    ...
    factor_col_idx = {f: [X_frame.columns.get_loc(c) for c in restricted_feature_group_fullschema(f)] for f in IN_SCOPE_FACTORS}
    ...
    payload = {
        ...
        "target": "decision",
        "features": f"exp_10 full-schema frame ({FRAME_TAG}, {len(X_frame.columns)} cols, MinMax + one-hot + missing-flags)",
        ...
        "note": "compare against exp_9's decision_kdm_ard_23col (0.608 CV, 0.680 held-out) and exp_6's 0.593 (19-col reference)",
    }
    with open(d / "metrics.json", "w") as f:
        json.dump(payload, f, indent=2)
    print(f"[decision_kdm_ard_{FRAME_TAG}] macro_f1={payload['macro_f1_mean']} (exp_9 23-col ARD CV was 0.608)")
