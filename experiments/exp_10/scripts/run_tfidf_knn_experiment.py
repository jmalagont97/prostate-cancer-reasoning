#!/usr/bin/env python3
"""
exp_10: TF-IDF + KNN classifier on clinical narrative text.

Preprocesses text with spaCy (lowercase, remove special chars, stopwords,
lemmatize), vectorizes with TF-IDF, and classifies biopsy decision via KNN.
Searches 288 configurations (72 KNN × 4 max_features) via 50 MCCV splits,
selects best by Macro-F1, evaluates via 88-fold LOO.

Usage:
    python3 experiments/exp_10/scripts/run_tfidf_knn_experiment.py
"""
...
RESULTS = Path("experiments/exp_10/results")
REPORTS = Path("experiments/exp_10/reports")
...
MAX_FEATURES_LIST = [500, 1000, 2000, None]
EPS = 1e-10
CLASS_NAMES = ["no", "yes"]
...
def load_spacy():
    ...
    # Enable only tokenizer + sentencizer (we do our own lemmatization)
    from spacy.lang.en import English
    nlp_full = spacy.load(SPACY_MODEL, disable=["ner", "parser"])
    print(f"  spaCy model: {SPACY_MODEL} v{nlp_full.meta.get('version', '?')}, spacy={SPACY_VERSION}")
    return nlp_full
...
def tfidf_fit_transform(texts_train, max_features=None):
    """Fit TF-IDF on training texts, return vectorized matrix."""
    vec = TfidfVectorizer(
...
def config_name(k, metric, weight, variant, max_features):
    mf_str = f"all" if max_features is None else str(max_features)
    return f"tfidf_mf{mf_str}_knn_n{k}_metric{metric}_weights{weight}_variant{variant}"
...
def run_mccv(texts_preprocessed, y, confidence, splits_df, nlp):
    configs = {}
            ...
            for metric in METRICS_LIST:
                for weight in WEIGHTS_LIST:
                    for variant in VARIANTS:
                        cn = config_name(k, metric, weight, variant, mf)
                        configs[cn] = {
                            "n_neighbors": k,
                            "metric": metric,
                            "weights": weight,
    ...
    for split_idx in range(50):
        ...
        # TF-IDF per max_features value (fit on train only)
        tfidf_cache = {}
        for mf in MAX_FEATURES_LIST:
            X_train, vec = tfidf_fit_transform(texts_train, max_features=mf)
            X_val = tfidf_transform(texts_val, vec)
            # Convert to dense for KNN (sparse euclidean is slow)
            X_train_dense = X_train.toarray().astype(np.float64)
            X_val_dense = X_val.toarray().astype(np.float64)
            tfidf_cache[mf] = (X_train_dense, X_val_dense)
...
def run_loo(texts_preprocessed, y, confidence, splits_df, best_cfg, best_cn):
    ...
    for fold_idx in range(88):
        ...
        X_train, vec = tfidf_fit_transform(texts_train, max_features=mf)
        X_test = tfidf_transform(texts_test, vec)
        X_train_dense = X_train.toarray().astype(np.float64)
        ...
        if variant == "standard":
            ...
            y_prob = knn.predict_proba(X_test_dense)[:, 1]
            ...
            knn.fit(X_train_dense, y_train, conf_train)
            y_prob = knn.predict_proba(X_test_dense)
            y_pred = (y_prob >= 0.5).astype(int)
...
def plot_confusion_matrices(cm_mccv, cm_loo, out_dir, best_cn):
    ...
    fig.suptitle(f"exp_10 — TF-IDF KNN: {best_cn}", fontsize=13, fontweight="bold")
...
def main():
    print("=" * 70)
    print("exp_10: TF-IDF + KNN on clinical narrative text")
    print("=" * 70)
    t_start = time.time()
    ...
    # ── MCCV ───────────────────────────────────────────────────────────────
    print("\n[4/7] MCCV search (288 configs × 50 splits)...")
    configs, summary, oof_mccv = run_mccv(texts_processed, y, conf, splits_df, nlp)
    ...
    sel = {
        ...
        "total_loo_folds": 88,
        "input_modality": "clinical_narrative_tfidf",
        "preprocessing": "lowercase, remove special chars, remove stopwords, lemmatize",
