"""Backfill macro-F1 into exp_6's already-completed confidence/weights results, per this
project's cross-experiment macro-F1 reporting initiative (2026-08-12). Decision already reports
macro_f1 (unchanged). Reveal has no exp_6 condition (added in exp_8).

Reproduces the EXACT same CV loop as run_signals.py (same RANDOM_STATE/hyperparameters/data), so
the existing ordinal_distance/decisive_set_f1 numbers are reproduced identically -- this script
only computes the ADDITIONAL macro-F1 metric and merges it into the existing metrics.json files
via read-modify-write, rather than overwriting them. run_signals.py itself is left untouched, so
exp_6's results remain reproducible from that exact, original code.

Confidence macro-F1: standard 3-class macro-F1 (uncertain/borderline/clear).
Weights macro-F1: per-factor 4-class macro-F1 (not_used/noted/important/decisive, labels=[0,1,2,3]
explicitly so a class absent from a factor's true labels still counts toward the macro average
at F1=0, the technically correct definition), averaged across the 9 in-scope factors.

Run with the project's own .venv (from the project root):
    .venv/Scripts/python.exe experiments/exp_6/scripts/backfill_macro_f1.py
"""
