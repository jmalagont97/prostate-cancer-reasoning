#!/usr/bin/env python3
"""
exp_6: KNN classifier on full MRI embedding (1024 dimensions).

No PCA, no pruning, no MinMaxScaler. Raw embeddings used directly.
Searches 72 KNN configurations via 50 MCCV splits, selects best by Macro-F1,
evaluates selected config via 88-fold LOO, and generates confusion matrix figures.

Usage:
    python3 experiments/exp_6/scripts/run_knn_image_embedding_experiment.py
"""
...
RESULTS = Path("experiments/exp_6/results")
REPORTS = Path("experiments/exp_6/reports")
...
def plot_confusion_matrices(cm_mccv, cm_loo, out_dir, best_cn):
    ...
    fig.suptitle(f"exp_6 — MRI Embedding KNN: {best_cn}", fontsize=13, fontweight="bold")
...
def main():
    print("=" * 70)
    print("exp_6: KNN on full MRI embedding (1024 dims, no scaling)")
    print("=" * 70)
    t_start = time.time()
    ...
    # ── MCCV ───────────────────────────────────────────────────────────────
    print("\n[3/7] MCCV search (72 configs x 50 splits)...")
    ...
    # ── LOO ────────────────────────────────────────────────────────────────
    print("\n[5/7] LOO evaluation (best config only)...")
    ...
    (out_dir / "hyperparameters.json").write_text(json.dumps({
        ...
        "variant": best_cfg["variant"],
        "preprocessing": "none (raw 1024-dim embedding)",
    }, indent=2))
