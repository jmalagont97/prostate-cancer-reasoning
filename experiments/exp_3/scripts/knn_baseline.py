# KNN baseline for CHIMERA Task 1.1 (exp_3).
# 128 configs (4 distances x 2 rules x 2 weights x 8 k) over 50 frozen MCCV splits.
# Selection by Macro-F1 (tie-break F1_yes, then balanced accuracy) on MCCV only.
...
def _transform_fold(df_train: pd.DataFrame, df_val: pd.DataFrame):
    tf = TabularTransformer(CONTINUOUS_COLS, CATEGORICAL_COLS).fit(df_train)
...
def run_loo(dfu: pd.DataFrame, cfg: dict) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, dict]:
    cid = config_id(cfg)
    rows = []
    pooled_t = []
    pooled_p = []
    ...
    return loo_df, cm_df, pooled_metrics, loo_df
...
def main() -> None:
    ...
    grid_meta = {
        ...
        "continuous_cols": len(CONTINUOUS_COLS), "categorical_cols": len(CATEGORICAL_COLS),
        "imputation": "none", "pruning": "none",
        "confidence_map": json.dumps(CONFIDENCE_MAP),
    }
    write_data_manifest(dfu, grid_meta)
    print(f"[data] usable={len(dfu)} configs={len(grid)}")
    ...
    loo_df, loo_cm_df, pooled_metrics, _ = run_loo(dfu, sel_cfg)
    ...
    # classification_report.csv: pooled MCCV + pooled LOO for selected config
