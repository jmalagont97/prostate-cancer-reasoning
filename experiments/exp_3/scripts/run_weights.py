"""exp_3: variable-weight models, 2 conditions = {official, restricted}, on the 19-column frame.

`fh` excluded (separate tool-revealed source, unchanged from exp_1/exp_2). MRI is deliberately
excluded from every per-factor restricted group (see DESIGN.md Section 2/9 -- cspca correlates
with the MRI signal slightly more than pirads does, so there's no principled basis to assign it
to one factor over the other; it only appears in the "official" full-scope condition). Reuses
chimera_task1.train_reasoning.repeated_out_of_fold_predict for the CV/scoring (no changes to
train_reasoning.py), same as exp_2's run_weights.py.

Run with the project's own .venv (from the project root):
    .venv/Scripts/python.exe experiments/exp_3/scripts/run_weights.py
"""
...
def run_official(ann, X_full) -> dict:
    ...
    for factor in IN_SCOPE_FACTORS:
        ...
        f1s_base.append(f1b)
    return {
        "model": "OvR logistic regression, all 9 in-scope factors on the same 19-column frame",
...
def run_restricted(ann, X_full) -> dict:
    ...
    for factor in IN_SCOPE_FACTORS:
        ...
        f1s_base.append(f1b)
    return {
        "model": "OvR logistic regression, one classifier per factor on its own restricted column(s) (MRI excluded)",
        "mean_ordinal_error": round(float(np.mean(dists)), 3),
        "mean_decisive_set_f1": round(float(np.mean(f1s)), 3),
        "mean_ordinal_error_baseline": round(float(np.mean(dists_base)), 3),
        "mean_decisive_set_f1_baseline": round(float(np.mean(f1s_base)), 3),
...
def write_metrics(condition: str, metrics: dict) -> None:
    ...
    payload = {"condition": condition, "target": "variable_weights", "features": "exp_3 19-column frame", **metrics}
