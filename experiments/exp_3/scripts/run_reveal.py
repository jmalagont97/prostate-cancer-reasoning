"""exp_3: reveal-sequence model, 1 condition, on the 19-column frame.

Reuses chimera_task1.train_reasoning's make_classifier and the CV/scoring pattern from its
eval_reveal() (adapted to return values instead of only printing), same as exp_2's run_reveal.py.

Run with the project's own .venv (from the project root):
    .venv/Scripts/python.exe experiments/exp_3/scripts/run_reveal.py
"""
...
def write_metrics(condition: str, metrics: dict) -> None:
    ...
    payload = {"condition": condition, "target": "reveal_sequence", "features": "exp_3 19-column frame", **metrics}
