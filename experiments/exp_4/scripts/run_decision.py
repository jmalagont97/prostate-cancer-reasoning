"""exp_4: decision model, same 8 models as exp_3, on the 16-column clinical-only frame (no MRI).

Reuses experiments/exp_3/scripts/{models.py,cv_utils.py} unchanged -- no MRI-specific logic
needed here since select_exp4_feature_frame() never touches the embedding.

Run with the project's own .venv (from the project root):
    .venv/Scripts/python.exe experiments/exp_4/scripts/run_decision.py
"""
...
def run_kdm_condition(feature_frame, y: np.ndarray) -> dict:
    ...
    from chimera_task1.features import build_preprocessor as _bp

    preprocessor = _bp(feature_frame)
...
def write_metrics(condition: str, metrics: dict) -> None:
    ...
    payload = {"condition": condition, "target": "decision", "features": "exp_4 16-column frame (clinical + comorbidity, no MRI)", **metrics}
...
def main() -> None:
    ...
    X = select_exp4_feature_frame(inp)
