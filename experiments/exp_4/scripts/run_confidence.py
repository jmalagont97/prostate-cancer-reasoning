"""exp_4: confidence model, same 8 models as exp_3, on the 16-column clinical-only frame (no MRI).

Run with the project's own .venv (from the project root):
    .venv/Scripts/python.exe experiments/exp_4/scripts/run_confidence.py
"""
...
def run_sklearn_condition(X: pd.DataFrame, y: np.ndarray, y_labels, model_name: str, clf, use_sample_weight: bool) -> dict:
...
def write_metrics(condition: str, metrics: dict) -> None:
    ...
    payload = {"condition": condition, "target": "confidence", "features": "exp_4 16-column frame (clinical + comorbidity, no MRI)", **metrics}
...
def main() -> None:
    ...
    write_metrics("confidence_kdm", run_kdm_condition(X, y_labels))
