"""exp_15: weights_kdm_occlusion retrained to include `fh` -- 10 factors instead of 9. Identical
pipeline to experiments/exp_6/scripts/loo_full_metrics_weights_kdm_occlusion.py (itself a faithful
LOO backfill of exp_6's original weights_kdm_occlusion) except: the 19-column frame gains a 20th
column (cli_fh_binary, joined in after select_exp3_feature_frame() -- that function itself is
unchanged, so every other experiment still gets exactly 19 columns); the shared decision-KDM
backbone is refit on this 20-column frame each fold (it needs cli_fh_binary as an available input
dimension for the occlusion mechanism to occlude when computing fh's own signal); the factor loop
runs over all 10 TASK1_FACTORS.

`fh` uses cli_fh_binary directly as an input feature, per the user's explicit choice (2026-08-19,
see DESIGN.md Section 1) -- fh was never actually revealed in any of the 91 labeled cases (0/91).

Run with the project's own .venv (from the project root):
    .venv/Scripts/python.exe experiments/exp_15/scripts/loo_full_metrics_weights_kdm_occlusion_10factor.py
"""
...
from kdm_backbone import fit_kdm_backbone, occlusion_delta  # noqa: E402
...
RANDOM_STATE = 0
N_CLASSES = 4
ALL_FACTORS = TASK1_FACTORS  # all 10
...
def main() -> None:
    ...
    X_frame["cli_fh_binary"] = inp_ann["cli_fh_binary"].values  # the one addition vs. exp_6
    print(f"n={n}, 19-col frame + fh: {X_frame.shape}, factors: {ALL_FACTORS}\n")
    ...
    factor_col_idx = {f: [X_frame.columns.get_loc(c) for c in restricted_feature_group(f, "flags")] for f in ALL_FACTORS}

    Y_all = np.stack([
        np.array([WEIGHT_RANK[label] for label in ann[weight_col(f)].values]) for f in ALL_FACTORS
    ], axis=1)  # (91, 10)

    oof_pred = np.full((n, len(ALL_FACTORS)), -1, dtype=int)
    factor_failed = set()
    ...
    for fold_i, (train_idx, test_idx) in enumerate(LeaveOneOut().split(np.arange(n))):
        ...
        # Decision backbone refit on the 20-column frame (incl. cli_fh_binary) -- needed so the
        # occlusion mechanism has fh's column to occlude; a minor deviation from exp_9's canonical
        # 19-col decision model, appropriate here since this experiment isn't about decision
        # performance, only about deriving fh's weight signal via the same shared-backbone trick.
    ...
    included_factors = [f for f in ALL_FACTORS if f not in factor_failed]
    per_factor = {}
    ...
    for j, factor in enumerate(ALL_FACTORS):
        ...
        onehot_proba = np.eye(N_CLASSES)[preds]
    ...
    print(f"\nPooled across {len(included_factors)}/{len(ALL_FACTORS)} factors x {n} cases = {len(all_true_pooled)} predictions")
    ...
    payload = {
        "condition": "weights_kdm_occlusion_10factor",
        "target": "variable_weights",
        "protocol": "leave-one-out (91-fold, pooled)",
        "features": "exp_3 19-column frame + cli_fh_binary (20 columns)",
        "model": "KDM decision backbone (shared, n_classes=2, refit on 20-col frame) + per-factor occlusion-delta signal + isotonic recalibration -- unchanged mechanism from exp_6, now includes fh",
        ...
        "n_factors_skipped": len(ALL_FACTORS) - len(included_factors),
        ...
        "auroc_brier_note": "AUROC/Brier are computed from a one-hot HARD-DECISION pseudo-"
                             "probability (1.0 on the predicted class) -- isotonic regression has "
                             "no native calibrated-probability output. Same caveat as the 9-factor "
                             "backfill.",
        "reference": {
            "exp6_9factor_loo_mean_ordinal_error": 0.390,
            "exp6_9factor_loo_mean_decisive_set_f1": 0.427,
    ...
    out_dir = RESULTS_DIR / "loo_full_metrics_weights_kdm_occlusion_10factor"
    out_dir.mkdir(parents=True, exist_ok=True)
    with open(out_dir / "metrics.json", "w") as f:
        json.dump(payload, f, indent=2)
    per_case_df.to_csv(out_dir / "per_case.csv", index=False)

    print(f"\n[weights_kdm_occlusion_10factor, LOO] mean_ordinal_error={payload['mean_ordinal_error']} "
          f"(9-factor reference 0.390) mean_decisive_set_f1={payload['mean_decisive_set_f1']} "
          f"(9-factor reference 0.427) mean_accuracy={payload['mean_accuracy']} "
