"""exp_8: exp_6/exp_7's shared-backbone decision+confidence+weights pipeline, on the 23-column
expanded frame with winning hyperparameters from search_hyperparameters_v3.py. Every readout
function (compute_signals, occlusion_delta, kernel_distance_contribution) and every recalibration
mechanism (isotonic increasing="auto", make_classifier() blend, per-factor try/except) is
byte-identical to exp_6/exp_7 -- only the backbone's feature frame and hyperparameters differ.
...
Run with the project's own .venv (from the project root):
    .venv/Scripts/python.exe experiments/exp_8/scripts/run_signals_v3.py
"""
...
def main() -> None:
    ...
    assert pd.notna(y_decision_labels).all(), "exp_8 assumes the annotated 91-case set has a decision label for every row"
    ...
    for repeat in range(N_REPEATS):
        ...
        repeat_metrics["decision_kdm_v3"]["macro_f1"].append(f1_score(y_decision, decision_preds, average="macro"))
    ...
    d = RESULTS_DIR / "decision_kdm_v3"
    d.mkdir(exist_ok=True)
    macro_f1s = repeat_metrics["decision_kdm_v3"]["macro_f1"]
    payload = {
        "condition": "decision_kdm_v3",
        "target": "decision",
        "features": "exp_8 23-column frame (psav/psap/isup/bmi added)",
        "model": f"KDM (memory-based, sigma-only trained), combined config: {winning_config}",
        "macro_f1_mean": round(float(np.mean(macro_f1s)), 3),
        "macro_f1_std": round(float(np.std(macro_f1s)), 3),
        "n_folds": N_REPEATS * N_SPLITS,
        "note": "compare against exp_6's decision_kdm_backbone (macro_f1=0.593) and exp_7's decision_kdm_v2 (0.622 CV / 0.490 held-out)",
    }
    with open(d / "metrics.json", "w") as f:
        json.dump(payload, f, indent=2)
    print(f"[decision_kdm_v3] macro_f1={payload['macro_f1_mean']} (exp_6 was 0.593; exp_7's combined was 0.622 CV / 0.490 held-out)")

    for cond in CONFIDENCE_CONDITIONS:
        d = RESULTS_DIR / f"{cond}_v3"
        d.mkdir(exist_ok=True)
        dists = repeat_metrics[cond]["ordinal_distance"]
        payload = {
            "condition": f"{cond}_v3",
            "target": "confidence",
            "features": "exp_8 23-column frame (psav/psap/isup/bmi added)",
            "model": f"exp_8 KDM backbone + signal recalibration, config: {winning_config}",
        ...
        with open(d / "metrics.json", "w") as f:
            json.dump(payload, f, indent=2)
        print(f"[{cond}_v3] ordinal_distance={payload['ordinal_distance_mean']} "
              f"(baseline 0.527, incumbent confidence_svm 0.468, exp_6's {cond}=?)")
    ...
    for cond in WEIGHT_CONDITIONS:
        ...
        with open(d / "metrics.json", "w") as f:
            json.dump(payload, f, indent=2)
        print(f"[{cond}_v3] mean_ordinal_error={payload['mean_ordinal_error']} (baseline 0.413, incumbent weights_svm 0.382/0.392)")
