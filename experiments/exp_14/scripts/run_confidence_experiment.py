#!/usr/bin/env python3
"""
exp_14 (corrected v4): ICI Confidence Tree — Canonical Evaluation

Predicts target_confidence (uncertain/borderline/clear) using DecisionTreeClassifier
on ICI (Inter-Classifier Inconsistency) from ConfidenceWeightedKNN base models.

CORRECTIONS from v3:
  - Base models use ConfidenceWeightedKNN (NOT StandardKNN), matching exp_4-11 winners.
  - KNNs are trained on target_biopsy_decision_binary (y_binary), NOT target_confidence.
  - Clinical confidence is used ONLY as neighbor weights in the fuzzy variant.
  - The evaluated sample's own confidence is NEVER used in its prediction.

Canonical evaluation:
  - Primary metric: MOE_abs = (1/3) * Σ_c mean_{i:y_i=c}(|pred_i - true_i|/2)
  - Tiebreaker: F1_macro
  - Baseline: always predict "clear" using REAL MCCV labels (exactly 900 predictions)
  - Selection: validity 100%, MOE_abs < baseline, no zero recall, minimize MOE_abs, tiebreak F1_macro
  - LOO: sanity check only, never for selection

Pipeline:
  1. Inner OOF: 3-fold CV within MCCV training set → p_T, p_M, p_X without circularity.
  2. ICI = 2 * |p_bar - 0.5| * (1 - 2*sigma).
  3. DecisionTreeClassifier(max_depth=2, max_leaf_nodes=3, min_samples_leaf=5)
     with class_weight as hyperparameter (None vs "balanced").
  4. MCCV (50 splits × 18 = 900 pooled predictions) for selection.
  5. LOO (88 folds) for sanity check.
"""
...
RESULTS_DIR = ROOT / "experiments" / "exp_14" / "results"
...
# ══════════════════════════════════════════════════════════════════════════════
# Confidence-weighted KNN (fuzzy variant, v2) — matches exp_12
# ══════════════════════════════════════════════════════════════════════════════

class ConfidenceWeightedKNN:
    """KNN with confidence-weighted fuzzy variant.

    Trained on target_biopsy_decision_binary (y_binary).
    Neighbor weights use target_confidence via: q = 0.5 + c_nn * (y_nn - 0.5).
    """
...
# ══════════════════════════════════════════════════════════════════════════════
# Tabular preprocessing (frozen 21 variables)
# ══════════════════════════════════════════════════════════════════════════════
...
def extract_two_thresholds(tree):
    th = tree.tree_.threshold
    ...
    def _collect(i):
        if children_left[i] == children_right[i]:
            return []
        return _collect(children_left[i]) + [th[i]] + _collect(children_right[i])

    return sorted(_collect(0))


# ══════════════════════════════════════════════════════════════════════════════
# Canonical metrics: MOE_abs + F1_macro + confusion matrices
# ══════════════════════════════════════════════════════════════════════════════
...
def compute_metrics(y_true_ord, y_pred_ord):
    ...
    recall_per_class = {}
    for c, name in enumerate(ORD_NAMES):
        row_sum = cm_abs[c, :].sum()
        recall_per_class[name] = float(cm_abs[c, c] / row_sum) if row_sum > 0 else 0.0

    return {
        "moe_abs": moe_abs,
        "f1_macro": f1_macro,
        "cm_abs": cm_abs.tolist(),
...
def plot_cm_mccv(cm_abs_dict, config_names, out_path, n_predictions):
    ...
    fig.suptitle(f"Confusion Matrices — MCCV Pooled (N={n_predictions})",
    ...
    for col, cfg_name in enumerate(config_names):
        ...
        _plot_cm(axes[0, col], cm_abs, f"{cfg_name} (counts)", cmap="YlOrRd")
        ...
        axes[1, col].set_title(f"{cfg_name} (row-normalized)", fontsize=11, fontweight="bold")
        axes[1, col].set_xlabel("Predicted", fontsize=10)
        axes[1, col].set_ylabel("True", fontsize=10)
...
def select_best_config(mccv_results, baseline_moe):
    ...
    for cfg_name, cfg_data in mccv_results.items():
        valid_rate = cfg_data["valid_rate"]
        pooled = cfg_data["pooled_metrics"]

        if valid_rate < 1.0:
            print(f"  {cfg_name}: EXCLUDED (valid_structure_rate={valid_rate:.0%} < 100%)")
            continue
        if pooled["moe_abs"] >= baseline_moe:
            print(f"  {cfg_name}: EXCLUDED (MOE_abs={pooled['moe_abs']:.4f} >= baseline={baseline_moe:.4f})")
            continue
        if pooled["has_zero_recall"]:
            print(f"  {cfg_name}: EXCLUDED (zero recall in some class)")
            continue

        candidates.append({
            "name": cfg_name,
            "moe_abs": pooled["moe_abs"],
            "f1_macro": pooled["f1_macro"],
        })

    if not candidates:
        print("\n  NO eligible configs. Returning no_eligible_config.")
    ...
    print(f"    MOE_abs: {winner['moe_abs']:.4f} (baseline: {baseline_moe:.4f})")
    print(f"    F1_macro: {winner['f1_macro']:.4f} (tiebreak)")
    if len(candidates) > 1:
        print(f"  Runner-up: {candidates[1]['name']} (MOE_abs={candidates[1]['moe_abs']:.4f})")
...
def main():
    ...
    print("=" * 70)
    print("[SEARCH] exp_14 | base_pipelines=3 | configs=2 | ConfidenceWeightedKNN")
    print("=" * 70)
    t_start = time.time()
    ...
    print(f"  Tabular: {X_tab_raw.shape[1]} features ({len(cat_cols)} categorical)")
    print(f"  MRI embedding: {X_emb.shape[1]} dims")
    print(f"  Confidence: clear={conf_counts.get('clear',0)}, "
          f"borderline={conf_counts.get('borderline',0)}, "
          f"uncertain={conf_counts.get('uncertain',0)}")
    ...
    # ── Configurations ─────────────────────────────────────────────────────
    TREE_CONFIGS = {
        "tree_none": {"class_weight": None},
        "tree_balanced": {"class_weight": "balanced"},
    }

    # ── MCCV: 50 splits ────────────────────────────────────────────────────
    print("\n" + "=" * 70)
    print(f"[SEARCH] exp_14 | base_pipelines=3 | configs={TOTAL_MCCV_CONFIGS} | "
          f"config=0/{TOTAL_MCCV_CONFIGS} | fold=0/{TOTAL_MCCV_FOLDS} | completed=0/{TOTAL_MCCV_CONFIGS * TOTAL_MCCV_FOLDS}")
    print("=" * 70)
    ...
    for cfg_idx, (config_name, config_params) in enumerate(TREE_CONFIGS.items(), 1):
        print(f"\n  === Config {cfg_idx}/{TOTAL_MCCV_CONFIGS}: {config_name} "
              f"(class_weight={config_params['class_weight']}) ===")
        per_fold = []
        all_preds = []
        t_config = time.time()
        ...
        for fold in range(TOTAL_MCCV_FOLDS):
            ...
            # Phase 1: Inner OOF for training ICI
            ...
            for k in range(INNER_N_SPLITS):
                inner_val_start = k * n_inner
                inner_val_end = (k + 1) * n_inner if k < INNER_N_SPLITS - 1 else len(train_idx)
                inner_val_idx = train_idx[inner_val_start:inner_val_end]
                inner_train_idx = np.concatenate([train_idx[:inner_val_start], train_idx[inner_val_end:]])
                ...
                p_t_oof[inner_val_start:inner_val_end] = knn_t.predict_proba(X_iv)
                ...
                p_m_oof[inner_val_start:inner_val_end] = knn_m.predict_proba(
                    pca.transform(X_emb[inner_val_idx]))
                ...
                p_x_oof[inner_val_start:inner_val_end] = knn_x.predict_proba(X_txt_iv)

            ici_train = compute_ici(p_t_oof, p_m_oof, p_x_oof)

            # Phase 2: Base models on full training → predict val
            ...
            # Phase 3: Train DecisionTreeClassifier
            dt = DecisionTreeClassifier(
                max_depth=2, max_leaf_nodes=3, min_samples_leaf=5,
                random_state=42, **{k: v for k, v in config_params.items() if v is not None}
            )
            dt.fit(ici_train.reshape(-1, 1), conf_ord[train_idx])

            # Phase 4: Extract thresholds
            n_nodes = dt.tree_.node_count
            n_leaves = dt.tree_.n_leaves
            thresholds = extract_two_thresholds(dt)
            valid_structure = (len(thresholds) == 2 and n_leaves == 3)

            # Phase 5: Predict
            y_pred_proba = dt.predict_proba(ici_val.reshape(-1, 1))
            ...
            for i, vi in enumerate(val_idx):
                all_preds.append({
                    "fold": fold,
                    "case_id": case_ids[vi],
                    "y_true": int(y_true_ord[i]),
                    "y_true_name": ORD_NAMES[int(y_true_ord[i])],
                    "y_pred": int(y_pred_ord[i]),
                    "y_pred_name": ORD_NAMES[int(y_pred_ord[i])],
                    "ici": float(ici_val[i]),
                    "t1": float(thresholds[0]) if len(thresholds) > 0 else np.nan,
                    "t2": float(thresholds[1]) if len(thresholds) > 1 else np.nan,
                })

            dt_time = time.time() - t_fold
            fold_counter += 1
            t1_str = f"{thresholds[0]:.4f}" if len(thresholds) > 0 else "N/A"
            t2_str = f"{thresholds[1]:.4f}" if len(thresholds) > 1 else "N/A"
            print(f"  [SEARCH] exp_14 | configs={TOTAL_MCCV_CONFIGS} | "
                  f"config={cfg_idx}/{TOTAL_MCCV_CONFIGS} | fold={fold+1}/{TOTAL_MCCV_FOLDS} | "
                  f"completed={fold_counter}/{TOTAL_MCCV_CONFIGS * TOTAL_MCCV_FOLDS} | "
                  f"moe={metrics['moe_abs']:.4f} f1m={metrics['f1_macro']:.3f} "
                  f"valid={valid_structure} t1={t1_str} t2={t2_str} ({dt_time:.1f}s)")

            per_fold.append({
                "fold": fold, "valid_structure": valid_structure,
                "n_nodes": n_nodes, "n_leaves": n_leaves,
                "t1": thresholds[0] if len(thresholds) > 0 else np.nan,
                "t2": thresholds[1] if len(thresholds) > 1 else np.nan,
                "moe_abs": metrics["moe_abs"],
                "f1_macro": metrics["f1_macro"],
            })

        # Pool predictions for this config
        ...
        valid_folds = [pf for pf in per_fold if pf["valid_structure"]]
        valid_rate = len(valid_folds) / len(per_fold)
        t1_vals = [pf["t1"] for pf in valid_folds]
        t2_vals = [pf["t2"] for pf in valid_folds]
        ...
        mccv_results[config_name] = {
            ...
            "t1_median": float(np.median(t1_vals)) if t1_vals else np.nan,
            "t2_median": float(np.median(t2_vals)) if t2_vals else np.nan,
        }

        mccv_all_predictions[config_name] = all_preds

        print(f"\n  {config_name}: MOE_abs={pooled_metrics['moe_abs']:.4f}  "
              f"F1m={pooled_metrics['f1_macro']:.4f}  "
              f"valid={len(valid_folds)}/{TOTAL_MCCV_FOLDS}  "
              f"t1_med={mccv_results[config_name]['t1_median']:.4f}  "
              f"t2_med={mccv_results[config_name]['t2_median']:.4f}  "
    ...
    if best_cfg is None:
        print("\n  WARNING: No eligible config. Recording no_eligible_config.")
        best_cfg = list(TREE_CONFIGS.keys())[0]
        best_params = TREE_CONFIGS[best_cfg]
    else:
        best_params = TREE_CONFIGS[best_cfg]

    # ── LOO on best config ─────────────────────────────────────────────────
    print("\n" + "=" * 70)
    print(f"[LOO] exp_14 | config={best_cfg} | fold=0/{TOTAL_LOO_FOLDS} | completed=0/{TOTAL_LOO_FOLDS}")
    print("=" * 70)
    ...
    for fold in range(TOTAL_LOO_FOLDS):
        ...
        dt = DecisionTreeClassifier(
            max_depth=2, max_leaf_nodes=3, min_samples_leaf=5,
            random_state=42, **{k: v for k, v in best_params.items() if v is not None}
        )
        dt.fit(ici_train.reshape(-1, 1), conf_ord[train_idx])
        ...
        # Predict
        pred_proba = dt.predict_proba(ici_test.reshape(-1, 1))
        ...
        loo_predictions.append({
            "fold": fold,
            "case_id": case_ids[test_idx],
            "y_true": true_ord,
            "y_true_name": ORD_NAMES[true_ord],
            "y_pred": pred_ord,
            "y_pred_name": ORD_NAMES[pred_ord],
            "ici": float(ici_test[0]),
            "t1": float(thresholds[0]) if len(thresholds) > 0 else np.nan,
            "t2": float(thresholds[1]) if len(thresholds) > 1 else np.nan,
        })
        ...
        print(f"  [LOO] exp_14 | config={best_cfg} | fold={fold+1}/{TOTAL_LOO_FOLDS} | "
    ...
    valid_loo = [pf for pf in loo_per_fold if pf["valid_structure"]]

    print(f"\n  LOO complete ({loo_time:.1f}s):")
    print(f"    Valid structure: {len(valid_loo)}/{TOTAL_LOO_FOLDS}")
    ...
    cfg_with_bl = list(TREE_CONFIGS.keys()) + ["baseline_clear"]
    n_pooled = sum(len(p) for p in mccv_all_predictions.values())
    plot_cm_mccv(cm_mccv_data, cfg_with_bl,
                 FIGURES_DIR / "confusion_matrices_mccv.png", n_pooled)

    plot_cm_loo(cm_loo_abs.tolist(),
                f"LOO Confusion Matrix — {best_cfg} (N=88)",
                FIGURES_DIR / "confusion_matrix_loo_selected.png")
    plot_cm_loo(cm_loo_norm.tolist(),
                f"LOO Normalized CM — {best_cfg} (N=88)",
                FIGURES_DIR / "confusion_matrix_loo_selected_normalized.png")
    plot_cm_loo(cm_loo_bl_abs.tolist(),
                "LOO Confusion Matrix — Baseline (always clear, N=88)",
                FIGURES_DIR / "confusion_matrix_loo_baseline.png")
    ...
    cm_all = {
        "mccv_pooled": {k: v for k, v in cm_mccv_data.items()},
        "loo_selected": {"config": best_cfg, "cm_abs": cm_loo_abs.tolist(), "cm_norm": cm_loo_norm.tolist()},
        "loo_baseline": {"cm_abs": cm_loo_bl_abs.tolist(), "cm_norm": cm_loo_bl_norm.tolist()},
    }
    ...
    summary = {
        "experiment": "exp_14_ici_tree_canonical_v4",
        "version": "v4_confidence_weighted",
        "ici_formula": "ICI = 2 * |p_bar - 0.5| * (1 - 2*sigma)",
        ...
        "mccv_selection": {
            ...
            "selection_rule": [
                "valid_structure_rate == 100%",
                "MOE_abs < baseline",
                "no zero recall",
                "minimize MOE_abs",
                "tiebreak: maximize F1_macro",
            ],
        },
        "loo_best_config": {
            "config": best_cfg,
            "params": best_params,
            "moe_abs": loo_metrics["moe_abs"],
            "f1_macro": loo_metrics["f1_macro"],
            "valid_structure_rate": len(valid_loo) / TOTAL_LOO_FOLDS,
        },
        "baseline_mccv": {
            "moe_abs": baseline_moe,
    ...
    for cfg_name, cfg_data in mccv_results.items():
        summary["configurations"][cfg_name] = {
            ...
            "valid_folds": cfg_data["valid_folds"],
            "valid_rate": cfg_data["valid_rate"],
            "t1_median": cfg_data["t1_median"],
            "t2_median": cfg_data["t2_median"],
        }
    ...
    rows = []
    for cfg_name, cfg_data in mccv_results.items():
        for pf in cfg_data["per_fold"]:
            rows.append({
                "config": cfg_name,
                "fold": pf["fold"],
                "valid_structure": pf["valid_structure"],
                "t1": pf["t1"], "t2": pf["t2"],
                "n_nodes": pf["n_nodes"], "n_leaves": pf["n_leaves"],
                "moe_abs": pf["moe_abs"],
                "f1_macro": pf["f1_macro"],
            })
    pd.DataFrame(rows).to_csv(RESULTS_DIR / "per_fold.csv", index=False)
    print(f"  Saved: per_fold.csv")

    mccv_pred_rows = []
    for cfg_name, preds in mccv_all_predictions.items():
        for p in preds:
            mccv_pred_rows.append({"config": cfg_name, **p})
    pd.DataFrame(mccv_pred_rows).to_csv(RESULTS_DIR / "predictions_mccv.csv", index=False)
    print(f"  Saved: predictions_mccv.csv")

    pd.DataFrame(loo_predictions).to_csv(RESULTS_DIR / "predictions_loo.csv", index=False)
    print(f"  Saved: predictions_loo.csv")
    ...
    for cfg_name, cfg_data in mccv_results.items():
        scorecard_rows.append({
            ...
            "moe_abs": cfg_data["pooled_metrics"]["moe_abs"],
            "f1_macro": cfg_data["pooled_metrics"]["f1_macro"],
            "n_predictions": cfg_data["pooled_metrics"]["n"],
            "valid_structure_rate": cfg_data["valid_rate"],
            "eligible": (cfg_data["valid_rate"] == 1.0 and
                         cfg_data["pooled_metrics"]["moe_abs"] < baseline_moe and
                         not cfg_data["pooled_metrics"]["has_zero_recall"]),
    ...
    maj_pred_loo_arr = np.full(len(loo_y_true_arr), ORD_MAP["clear"], dtype=int)
    bl_loo_metrics = compute_metrics(loo_y_true_arr, maj_pred_loo_arr)
    scorecard_rows.append({
        "config": "baseline_clear",
        "scope": "baseline_LOO",
        "moe_abs": bl_loo_metrics["moe_abs"],
        "f1_macro": bl_loo_metrics["f1_macro"],
        "n_predictions": bl_loo_metrics["n"],
        "valid_structure_rate": 1.0,
        "eligible": "",
        "selection_status": "",
    })
    ...
    print("\n" + "=" * 70)
    print("[SUMMARY] exp_14 (v4: ConfidenceWeightedKNN)")
    print("=" * 70)
    ...
    for cfg_name, cfg_data in mccv_results.items():
        ...
        print(f"    {cfg_name}{tag}: MOE_abs={pm['moe_abs']:.4f}  F1m={pm['f1_macro']:.4f}  "
              f"valid={cfg_data['valid_folds']}/{TOTAL_MCCV_FOLDS}")
