"""exp_14: pooled confusion matrix + classification report for the joint (dim_y=9) KDM
regression condition, scored via leave-one-out (91 folds, ONE fit per fold covering all 9
factors), pooling every (case, factor) prediction into one confusion matrix over the 4 ordinal
weight levels. LOO wasn't part of the original staged plan, run here on explicit request.

Also saves a per-case breakdown (one row per case_id: each factor's true/predicted rank, plus a
per-case mean ordinal error and exact-match count out of 9) -- see
loo_confusion_matrix_per_factor.py's module docstring for the rationale.

Run with the project's own .venv (from the project root):
    .venv/Scripts/python.exe experiments/exp_14/scripts/loo_confusion_matrix_joint.py
"""
...
def main() -> None:
    ...
    for fold_i, (train_idx, test_idx) in enumerate(LeaveOneOut().split(idx_all)):
        ...
        pred_row = sig["pred_rank"][0]  # (9,)
        true_row = Y_all[test_idx[0]]  # (9,)
        all_true.extend(int(v) for v in true_row)
        all_pred.extend(int(v) for v in pred_row)

        case_row = {"case_id": case_ids[test_idx[0]]}
        abs_errors = []
        for j, factor in enumerate(IN_SCOPE_FACTORS):
            true_r, pred_r = int(true_row[j]), int(pred_row[j])
    ...
    print(f"\nPooled across all {len(IN_SCOPE_FACTORS)} factors x {n} cases = {len(all_true)} predictions "
          f"(from {n} joint fits, not {n * DIM_Y})")
    ...
    out = {
        ...
        "protocol": "leave-one-out (91-fold, one joint fit per fold, pooled across factors)",
    ...
    out_dir = RESULTS_DIR / "loo_confusion_matrix_weights_regress_joint"
