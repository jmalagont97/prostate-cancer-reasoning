"""exp_26 — MemKDM without the noise-selected linear encoder. See ../DESIGN.md and ../IMPLEMENTATION.md.

Corrective follow-up to exp_25: exp_25's Stage 1 grid search picked `encoder=linear` for `tab` by a
margin (0.003) far smaller than each config's own MCCV noise (std ~0.11) — a selection `select_best`'s
own tie-break docstring flags but doesn't catch (tie-break only fires within tol=1e-9). `linear`'s
trainable random init made every downstream model stochastic and regressed `unimodal_tab`'s LOOCV
Macro-F1 to 0.5978, ~0.07 below exp_23 Arm B's deterministic 0.6694 (same sigma_mult=2.0, but
encoder=identity) that exp_25 itself exactly reproduces elsewhere in the same run. This experiment drops
`linear` from the grid entirely and, per DESIGN.md §2.3, resolves the winning `sigma_mult` into an
explicit fold-local `sigma` when transferring it from Stage 1 into Stage 2/Phase B (verified a numerical
no-op for `encoder=identity`; makes the transfer explicit instead of implicit inside `MemKDM.fit()`).

Everything not called out in DESIGN.md/IMPLEMENTATION.md is copied verbatim from
`exp_25/scripts/train.py`, including its own two build-time simplifications (no per-config feature
caching; fusion-weight stash replaced by a small post-hoc re-fit pass) — see that file's docstring.
"""
...
from kdm.init import _sigma_from_knn  # noqa: E402
...
TAB_GRID = [
    {"sigma_mult": sm, "encoder": "identity", "y_train": yt}
    for sm in [0.25, 0.5, 1.0, 2.0] for yt in [False, True]
]
MRI_GRID = [
    {"rep": rep, "sigma_mult": sm, "encoder": "identity", "y_train": yt}
    for rep in ["raw_l2", "pca90_l2"] for sm in [0.5, 1.0, 2.0] for yt in [False, True]
]
TXT_GRID = [
    {"rep": (mf, 0.90), "sigma_mult": sm, "encoder": "identity", "y_train": yt}
...
def main(smoke: bool = False):
    ...
    print(f"[exp_26] cohort N={n}, yes={int(cohort.y_binary.sum())}, no={int((1 - cohort.y_binary).sum())}")

    print("[exp_26] spaCy cleaning text corpus (once, cohort-level)...")
    ...
    print(f"[exp_26] {'SMOKE MODE: ' if smoke else ''}{len(SPLITS_3T)} MCCV splits, {len(LOOCV_FOLDS)} LOOCV folds")
    ...
    print("[exp_26] Step 0: reproduction gate...")
    ...
    print(f"[exp_26] Step 0 PASSED: macro_f1={repro_metrics['macro_f1']!r}, "
    ...
    print("[exp_26] Step 0: check_roundtrip passed for tab / mri(pca90_l2) / txt(2000,0.9)")

    # ---------------------------------------------------------------- 1.4 Stage 1 Phase A
    print("[exp_26] Stage 1 Phase A...")
    ...
    for name, grid in [("tab", TAB_GRID), ("mri", MRI_GRID), ("txt", TXT_GRID)]:
        ...
        print(f"[exp_26] Stage 1 {name}: best mean_macro_f1={STAGE1_BEST[name]['mean_macro_f1']:.4f}, cfg={STAGE1_BEST[name]}")
    ...
    print("[exp_26] Fusion-weight search (post-hoc re-fit of the 3 Stage-1 winners across 100 splits)...")
    ...
    print(f"[exp_26] fusion weights: {fusion_weights}")

    # ---------------------------------------------------------------- 1.5 Stage 2 Phase A
    print("[exp_26] Stage 2 Phase A...")

    def build_joint_kernels_encoders(subset, cfg, X_tr):
        """Transfers Stage 1's winning `sigma_mult` forward as an explicit, fold-local `sigma` (resolved
        from THIS fold's own X_tr only — leak-free, see DESIGN.md §2.3) instead of re-passing sigma_mult
        for MemKDM.fit() to re-derive internally. Numerically a no-op vs. exp_25 for encoder=identity."""
        kernels, encoders = {}, {}
        for mod in subset:
    ...
    def stage2_evaluate_factory(subset, label_smoothing=0.0):
        def evaluate_fn(cfg, train_idx, val_idx):
            ...
            kernels, encoders = build_joint_kernels_encoders(subset, cfg, X_tr)
    ...
    for subset in CONDITIONS:
        ...
        print(f"[exp_26] Stage 2 {key}: best mean_macro_f1={STAGE2_BEST[key]['mean_macro_f1']:.4f}")
    ...
    print(f"[exp_26] Stage 2 confidence_arm: best mean_macro_f1={STAGE2_BEST['confidence_arm']['mean_macro_f1']:.4f}")
    ...
    for mod in ["tab", "mri", "txt"]:
        ...
        unimodal_oof[mod] = phase_b_condition([mod], cfg, n_seeds=1)  # encoder=identity everywhere -> deterministic
        print(f"[exp_26] Phase B unimodal_{mod}: macro_f1={np.mean(unimodal_oof[mod]['per_seed_macro_f1']):.4f}")

    joint_oof = {}
    for key, subset in JOINT_KEYS.items():
        cfg = STAGE2_BEST[key]
        joint_oof[key] = phase_b_condition(subset, cfg, n_seeds=1)
        print(f"[exp_26] Phase B {key}: macro_f1={np.mean(joint_oof[key]['per_seed_macro_f1']):.4f}")

    cfg_c = STAGE2_BEST["confidence_arm"]
    confidence_arm_oof = phase_b_condition(["tab", "mri", "txt"], cfg_c, n_seeds=1, label_smoothing=0.10)
    print("[exp_26] Phase B confidence_arm: done")
    ...
    # ---------------------------------------------------------------- figures
    print("[exp_26] Figures...")
    try:
        for name, df_grid in [("tab", STAGE1_GRIDS[0]), ("mri", STAGE1_GRIDS[1]), ("txt", STAGE1_GRIDS[2])]:
            plot_grid_search_curves(df_grid, x_col="sigma_mult", y_col="mean_macro_f1", group_cols=["y_train"],
                                     title=f"exp_26 Stage 1 {name} grid search", out_path=FIG_DIR / f"stage1_grid_search_curves_{name}.png")
        for df_grid in STAGE2_GRIDS:
            ...
            plot_grid_search_curves(df_grid, x_col="sigma_scale", y_col="mean_macro_f1", group_cols=["kernel_trainable", "y_train"],
                                     title=f"exp_26 Stage 2 {cond} grid search", out_path=FIG_DIR / f"stage2_grid_search_curves_{cond}.png")
        plot_confusion_matrix(y_true_eval, y_pred_trimodal, labels=["No", "Yes"],
                               title="exp_26 joint_trimodal LOOCV", out_path=FIG_DIR / "confusion_matrix.png")
        plot_roc_curves({"joint_trimodal": ALL_CONDITIONS["joint_trimodal"][idx, 1],
                         ...
                         y_true_eval, title="exp_26 ROC overlay", out_path=FIG_DIR / "roc_curve.png")
        plot_signal_scatter(CONDITION_SIGNALS["joint_trimodal"]["h_aleatoric"][idx], CONDITION_SIGNALS["joint_trimodal"]["h_epistemic"][idx],
                             ...
                             xlabel="h_aleatoric", ylabel="h_epistemic", title="exp_26 joint_trimodal particle signals",
        ...
        print(f"[exp_26] best confidence row (for reference, not re-plotted): {best_conf_row['condition']}/{best_conf_row['head']} "
        ...
        print(f"[exp_26] WARNING: figure generation failed: {e}")

    record_git_commit(RESULTS_DIR)
    elapsed = time.time() - t_start
    print(f"[exp_26] DONE in {elapsed / 60:.1f} min. Results in {RESULTS_DIR}")
